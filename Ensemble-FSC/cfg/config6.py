...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg6 in yaml_cfg.setdefault('BASE', ['']):
        if cfg6:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg6)
            )
...
def update_config(config, args6):
    _update_config_from_file(config, args6.cfg6)

    config.defrost()
    if args6.opts6:
        config.merge_from_list(args6.opts6)

    def _check_args(name):
        if hasattr(args6, name) and eval(f'args6.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train6'):
        config.IS_TRAIN = args6.is_train6
    if _check_args('train_batch_size6'):
        config.DATA.TRAIN.BATCH_SIZE = args6.train_batch_size6
    if _check_args('valid_batch_size6'):
        config.DATA.VALID.BATCH_SIZE = args6.valid_batch_size6
    if _check_args('test_batch_size6'):
        config.DATA.TEST.BATCH_SIZE = args6.test_batch_size6
    if _check_args('pretrained6'):
        config.MODEL.PRETRAINED = args6.pretrained6
    if _check_args('resume6'):
        config.MODEL.RESUME = args6.resume6
    if _check_args('output6'):
        config.OUTPUT = args6.output6
    if _check_args('tag6'):
        config.TAG = args6.tag6
...
def get_config6(args6):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args6)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg5 in yaml_cfg.setdefault('BASE', ['']):
        if cfg5:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg5)
            )
...
def update_config(config, args5):
    _update_config_from_file(config, args5.cfg5)

    config.defrost()
    if args5.opts5:
        config.merge_from_list(args5.opts5)

    def _check_args(name):
        if hasattr(args5, name) and eval(f'args5.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train5'):
        config.IS_TRAIN = args5.is_train5
    if _check_args('train_batch_size5'):
        config.DATA.TRAIN.BATCH_SIZE = args5.train_batch_size5
    if _check_args('valid_batch_size5'):
        config.DATA.VALID.BATCH_SIZE = args5.valid_batch_size5
    if _check_args('test_batch_size5'):
        config.DATA.TEST.BATCH_SIZE = args5.test_batch_size5
    if _check_args('pretrained5'):
        config.MODEL.PRETRAINED = args5.pretrained5
    if _check_args('resume5'):
        config.MODEL.RESUME = args5.resume5
    if _check_args('output5'):
        config.OUTPUT = args5.output5
    if _check_args('tag5'):
        config.TAG = args5.tag5
...
def get_config5(args5):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args5)

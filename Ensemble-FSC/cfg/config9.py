...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg9 in yaml_cfg.setdefault('BASE', ['']):
        if cfg9:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg9)
            )
...
def update_config(config, args9):
    _update_config_from_file(config, args9.cfg9)

    config.defrost()
    if args9.opts9:
        config.merge_from_list(args9.opts9)

    def _check_args(name):
        if hasattr(args9, name) and eval(f'args9.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train9'):
        config.IS_TRAIN = args9.is_train9
    if _check_args('train_batch_size9'):
        config.DATA.TRAIN.BATCH_SIZE = args9.train_batch_size9
    if _check_args('valid_batch_size9'):
        config.DATA.VALID.BATCH_SIZE = args9.valid_batch_size9
    if _check_args('test_batch_size9'):
        config.DATA.TEST.BATCH_SIZE = args9.test_batch_size9
    if _check_args('pretrained9'):
        config.MODEL.PRETRAINED = args9.pretrained9
    if _check_args('resume9'):
        config.MODEL.RESUME = args9.resume9
    if _check_args('output9'):
        config.OUTPUT = args9.output9
    if _check_args('tag9'):
        config.TAG = args9.tag9
...
def get_config9(args9):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args9)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg4 in yaml_cfg.setdefault('BASE', ['']):
        if cfg4:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg4)
            )
...
def update_config(config, args4):
    _update_config_from_file(config, args4.cfg4)

    config.defrost()
    if args4.opts4:
        config.merge_from_list(args4.opts4)

    def _check_args(name):
        if hasattr(args4, name) and eval(f'args4.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train4'):
        config.IS_TRAIN = args4.is_train4
    if _check_args('train_batch_size4'):
        config.DATA.TRAIN.BATCH_SIZE = args4.train_batch_size4
    if _check_args('valid_batch_size4'):
        config.DATA.VALID.BATCH_SIZE = args4.valid_batch_size4
    if _check_args('test_batch_size4'):
        config.DATA.TEST.BATCH_SIZE = args4.test_batch_size4
    if _check_args('pretrained4'):
        config.MODEL.PRETRAINED = args4.pretrained4
    if _check_args('resume4'):
        config.MODEL.RESUME = args4.resume4
    if _check_args('output4'):
        config.OUTPUT = args4.output4
    if _check_args('tag4'):
        config.TAG = args4.tag4
...
def get_config4(args4):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args4)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg8 in yaml_cfg.setdefault('BASE', ['']):
        if cfg8:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg8)
            )
...
def update_config(config, args8):
    _update_config_from_file(config, args8.cfg8)

    config.defrost()
    if args8.opts8:
        config.merge_from_list(args8.opts8)

    def _check_args(name):
        if hasattr(args8, name) and eval(f'args8.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train8'):
        config.IS_TRAIN = args8.is_train8
    if _check_args('train_batch_size8'):
        config.DATA.TRAIN.BATCH_SIZE = args8.train_batch_size8
    if _check_args('valid_batch_size8'):
        config.DATA.VALID.BATCH_SIZE = args8.valid_batch_size8
    if _check_args('test_batch_size8'):
        config.DATA.TEST.BATCH_SIZE = args8.test_batch_size8
    if _check_args('pretrained8'):
        config.MODEL.PRETRAINED = args8.pretrained8
    if _check_args('resume8'):
        config.MODEL.RESUME = args8.resume8
    if _check_args('output8'):
        config.OUTPUT = args8.output8
    if _check_args('tag8'):
        config.TAG = args8.tag8
...
def get_config8(args8):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args8)

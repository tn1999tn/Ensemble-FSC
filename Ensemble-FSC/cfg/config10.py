...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg10 in yaml_cfg.setdefault('BASE', ['']):
        if cfg10:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg10)
            )
...
def update_config(config, args10):
    _update_config_from_file(config, args10.cfg10)

    config.defrost()
    if args10.opts10:
        config.merge_from_list(args10.opts10)

    def _check_args(name):
        if hasattr(args10, name) and eval(f'args10.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train10'):
        config.IS_TRAIN = args10.is_train10
    if _check_args('train_batch_size10'):
        config.DATA.TRAIN.BATCH_SIZE = args10.train_batch_size10
    if _check_args('valid_batch_size10'):
        config.DATA.VALID.BATCH_SIZE = args10.valid_batch_size10
    if _check_args('test_batch_size10'):
        config.DATA.TEST.BATCH_SIZE = args10.test_batch_size10
    if _check_args('pretrained10'):
        config.MODEL.PRETRAINED = args10.pretrained10
    if _check_args('resume10'):
        config.MODEL.RESUME = args10.resume10
    if _check_args('output10'):
        config.OUTPUT = args10.output10
    if _check_args('tag10'):
        config.TAG = args10.tag10
...
def get_config10(args10):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args10)

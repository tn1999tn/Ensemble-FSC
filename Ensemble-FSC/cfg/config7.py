...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg7 in yaml_cfg.setdefault('BASE', ['']):
        if cfg7:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg7)
            )
...
def update_config(config, args7):
    _update_config_from_file(config, args7.cfg7)

    config.defrost()
    if args7.opts7:
        config.merge_from_list(args7.opts7)

    def _check_args(name):
        if hasattr(args7, name) and eval(f'args7.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train7'):
        config.IS_TRAIN = args7.is_train7
    if _check_args('train_batch_size7'):
        config.DATA.TRAIN.BATCH_SIZE = args7.train_batch_size7
    if _check_args('valid_batch_size7'):
        config.DATA.VALID.BATCH_SIZE = args7.valid_batch_size7
    if _check_args('test_batch_size7'):
        config.DATA.TEST.BATCH_SIZE = args7.test_batch_size7
    if _check_args('pretrained7'):
        config.MODEL.PRETRAINED = args7.pretrained7
    if _check_args('resume7'):
        config.MODEL.RESUME = args7.resume7
    if _check_args('output7'):
        config.OUTPUT = args7.output7
    if _check_args('tag7'):
        config.TAG = args7.tag7
...
def get_config7(args7):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args7)

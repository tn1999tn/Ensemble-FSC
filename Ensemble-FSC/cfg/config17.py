...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg17 in yaml_cfg.setdefault('BASE', ['']):
        if cfg17:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg17)
            )
...
def update_config(config, args17):
    _update_config_from_file(config, args17.cfg17)

    config.defrost()
    if args17.opts17:
        config.merge_from_list(args17.opts17)

    def _check_args(name):
        if hasattr(args17, name) and eval(f'args17.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train17'):
        config.IS_TRAIN = args17.is_train17
    if _check_args('train_batch_size17'):
        config.DATA.TRAIN.BATCH_SIZE = args17.train_batch_size17
    if _check_args('valid_batch_size17'):
        config.DATA.VALID.BATCH_SIZE = args17.valid_batch_size17
    if _check_args('test_batch_size17'):
        config.DATA.TEST.BATCH_SIZE = args17.test_batch_size17
    if _check_args('pretrained17'):
        config.MODEL.PRETRAINED = args17.pretrained17
    if _check_args('resume17'):
        config.MODEL.RESUME = args17.resume17
    if _check_args('output17'):
        config.OUTPUT = args17.output17
    if _check_args('tag17'):
        config.TAG = args17.tag17
...
def get_config17(args17):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args17)

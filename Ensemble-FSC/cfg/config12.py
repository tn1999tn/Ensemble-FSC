...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg12 in yaml_cfg.setdefault('BASE', ['']):
        if cfg12:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg12)
            )
...
def update_config(config, args12):
    _update_config_from_file(config, args12.cfg12)

    config.defrost()
    if args12.opts12:
        config.merge_from_list(args12.opts12)

    def _check_args(name):
        if hasattr(args12, name) and eval(f'args12.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train12'):
        config.IS_TRAIN = args12.is_train12
    if _check_args('train_batch_size12'):
        config.DATA.TRAIN.BATCH_SIZE = args12.train_batch_size12
    if _check_args('valid_batch_size12'):
        config.DATA.VALID.BATCH_SIZE = args12.valid_batch_size12
    if _check_args('test_batch_size12'):
        config.DATA.TEST.BATCH_SIZE = args12.test_batch_size12
    if _check_args('pretrained12'):
        config.MODEL.PRETRAINED = args12.pretrained12
    if _check_args('resume12'):
        config.MODEL.RESUME = args12.resume12
    if _check_args('output12'):
        config.OUTPUT = args12.output12
    if _check_args('tag12'):
        config.TAG = args12.tag12
...
def get_config12(args12):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args12)

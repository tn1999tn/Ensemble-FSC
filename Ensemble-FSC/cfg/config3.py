...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg3 in yaml_cfg.setdefault('BASE', ['']):
        if cfg3:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg3)
            )
...
def update_config(config, args3):
    _update_config_from_file(config, args3.cfg3)

    config.defrost()
    if args3.opts3:
        config.merge_from_list(args3.opts3)

    def _check_args(name):
        if hasattr(args3, name) and eval(f'args3.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train3'):
        config.IS_TRAIN = args3.is_train3
    if _check_args('train_batch_size3'):
        config.DATA.TRAIN.BATCH_SIZE = args3.train_batch_size3
    if _check_args('valid_batch_size3'):
        config.DATA.VALID.BATCH_SIZE = args3.valid_batch_size3
    if _check_args('test_batch_size3'):
        config.DATA.TEST.BATCH_SIZE = args3.test_batch_size3
    if _check_args('pretrained3'):
        config.MODEL.PRETRAINED = args3.pretrained3
    if _check_args('resume3'):
        config.MODEL.RESUME = args3.resume3
    if _check_args('output3'):
        config.OUTPUT = args3.output3
    if _check_args('tag3'):
        config.TAG = args3.tag3
...
def get_config3(args3):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args3)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg2 in yaml_cfg.setdefault('BASE', ['']):
        if cfg2:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg2)
            )
...
def update_config(config, args2):
    _update_config_from_file(config, args2.cfg2)

    config.defrost()
    if args2.opts2:
        config.merge_from_list(args2.opts2)

    def _check_args(name):
        if hasattr(args2, name) and eval(f'args2.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train2'):
        config.IS_TRAIN = args2.is_train2
    if _check_args('train_batch_size2'):
        config.DATA.TRAIN.BATCH_SIZE = args2.train_batch_size2
    if _check_args('valid_batch_size2'):
        config.DATA.VALID.BATCH_SIZE = args2.valid_batch_size2
    if _check_args('test_batch_size2'):
        config.DATA.TEST.BATCH_SIZE = args2.test_batch_size2
    if _check_args('pretrained2'):
        config.MODEL.PRETRAINED = args2.pretrained2
    if _check_args('resume2'):
        config.MODEL.RESUME = args2.resume2
    if _check_args('output2'):
        config.OUTPUT = args2.output2
    if _check_args('tag2'):
        config.TAG = args2.tag2
...
def get_config2(args2):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args2)

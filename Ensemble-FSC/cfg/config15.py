...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg15 in yaml_cfg.setdefault('BASE', ['']):
        if cfg15:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg15)
            )
...
def update_config(config, args15):
    _update_config_from_file(config, args15.cfg15)

    config.defrost()
    if args15.opts15:
        config.merge_from_list(args15.opts15)

    def _check_args(name):
        if hasattr(args15, name) and eval(f'args15.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train15'):
        config.IS_TRAIN = args15.is_train15
    if _check_args('train_batch_size15'):
        config.DATA.TRAIN.BATCH_SIZE = args15.train_batch_size15
    if _check_args('valid_batch_size15'):
        config.DATA.VALID.BATCH_SIZE = args15.valid_batch_size15
    if _check_args('test_batch_size15'):
        config.DATA.TEST.BATCH_SIZE = args15.test_batch_size15
    if _check_args('pretrained15'):
        config.MODEL.PRETRAINED = args15.pretrained15
    if _check_args('resume15'):
        config.MODEL.RESUME = args15.resume15
    if _check_args('output15'):
        config.OUTPUT = args15.output15
    if _check_args('tag15'):
        config.TAG = args15.tag15
...
def get_config15(args15):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args15)

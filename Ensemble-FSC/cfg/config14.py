...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg14 in yaml_cfg.setdefault('BASE', ['']):
        if cfg14:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg14)
            )
...
def update_config(config, args14):
    _update_config_from_file(config, args14.cfg14)

    config.defrost()
    if args14.opts14:
        config.merge_from_list(args14.opts14)

    def _check_args(name):
        if hasattr(args14, name) and eval(f'args14.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train14'):
        config.IS_TRAIN = args14.is_train14
    if _check_args('train_batch_size14'):
        config.DATA.TRAIN.BATCH_SIZE = args14.train_batch_size14
    if _check_args('valid_batch_size14'):
        config.DATA.VALID.BATCH_SIZE = args14.valid_batch_size14
    if _check_args('test_batch_size14'):
        config.DATA.TEST.BATCH_SIZE = args14.test_batch_size14
    if _check_args('pretrained14'):
        config.MODEL.PRETRAINED = args14.pretrained14
    if _check_args('resume14'):
        config.MODEL.RESUME = args14.resume14
    if _check_args('output14'):
        config.OUTPUT = args14.output14
    if _check_args('tag14'):
        config.TAG = args14.tag14
...
def get_config14(args14):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args14)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg11 in yaml_cfg.setdefault('BASE', ['']):
        if cfg11:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg11)
            )
...
def update_config(config, args11):
    _update_config_from_file(config, args11.cfg11)

    config.defrost()
    if args11.opts11:
        config.merge_from_list(args11.opts11)

    def _check_args(name):
        if hasattr(args11, name) and eval(f'args11.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train11'):
        config.IS_TRAIN = args11.is_train11
    if _check_args('train_batch_size11'):
        config.DATA.TRAIN.BATCH_SIZE = args11.train_batch_size11
    if _check_args('valid_batch_size11'):
        config.DATA.VALID.BATCH_SIZE = args11.valid_batch_size11
    if _check_args('test_batch_size11'):
        config.DATA.TEST.BATCH_SIZE = args11.test_batch_size11
    if _check_args('pretrained11'):
        config.MODEL.PRETRAINED = args11.pretrained11
    if _check_args('resume11'):
        config.MODEL.RESUME = args11.resume11
    if _check_args('output11'):
        config.OUTPUT = args11.output11
    if _check_args('tag11'):
        config.TAG = args11.tag11
...
def get_config11(args11):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args11)

...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg1 in yaml_cfg.setdefault('BASE', ['']):
        if cfg1:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg1)
            )
...
def update_config(config, args1):
    _update_config_from_file(config, args1.cfg1)

    config.defrost()
    if args1.opts1:
        config.merge_from_list(args1.opts1)

    def _check_args(name):
        if hasattr(args1, name) and eval(f'args1.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train1'):
        config.IS_TRAIN = args1.is_train1
    if _check_args('train_batch_size1'):
        config.DATA.TRAIN.BATCH_SIZE = args1.train_batch_size1
    if _check_args('valid_batch_size1'):
        config.DATA.VALID.BATCH_SIZE = args1.valid_batch_size1
    if _check_args('test_batch_size1'):
        config.DATA.TEST.BATCH_SIZE = args1.test_batch_size1
    if _check_args('pretrained1'):
        config.MODEL.PRETRAINED = args1.pretrained1
    if _check_args('resume1'):
        config.MODEL.RESUME = args1.resume1
    if _check_args('output1'):
        config.OUTPUT = args1.output1
    if _check_args('tag1'):
        config.TAG = args1.tag1

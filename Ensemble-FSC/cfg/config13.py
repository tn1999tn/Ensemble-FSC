...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg13 in yaml_cfg.setdefault('BASE', ['']):
        if cfg13:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg13)
            )
...
def update_config(config, args13):
    _update_config_from_file(config, args13.cfg13)

    config.defrost()
    if args13.opts13:
        config.merge_from_list(args13.opts13)

    def _check_args(name):
        if hasattr(args13, name) and eval(f'args13.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train13'):
        config.IS_TRAIN = args13.is_train13
    if _check_args('train_batch_size13'):
        config.DATA.TRAIN.BATCH_SIZE = args13.train_batch_size13
    if _check_args('valid_batch_size13'):
        config.DATA.VALID.BATCH_SIZE = args13.valid_batch_size13
    if _check_args('test_batch_size13'):
        config.DATA.TEST.BATCH_SIZE = args13.test_batch_size13
    if _check_args('pretrained13'):
        config.MODEL.PRETRAINED = args13.pretrained13
    if _check_args('resume13'):
        config.MODEL.RESUME = args13.resume13
    if _check_args('output13'):
        config.OUTPUT = args13.output13
    if _check_args('tag13'):
        config.TAG = args13.tag13
...
def get_config13(args13):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args13)

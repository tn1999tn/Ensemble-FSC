...
def _update_config_from_file(config, cfg_file):
    ...
    #递归找base file
    for cfg16 in yaml_cfg.setdefault('BASE', ['']):
        if cfg16:
            _update_config_from_file(
                config, os.path.join(os.path.dirname(cfg_file), cfg16)
            )
...
def update_config(config, args16):
    _update_config_from_file(config, args16.cfg16)

    config.defrost()
    if args16.opts16:
        config.merge_from_list(args16.opts9)

    def _check_args(name):
        if hasattr(args16, name) and eval(f'args16.{name}') is not None:
            return True
        return False

    # merge from specific arguments
    if _check_args('is_train16'):
        config.IS_TRAIN = args16.is_train16
    if _check_args('train_batch_size16'):
        config.DATA.TRAIN.BATCH_SIZE = args16.train_batch_size16
    if _check_args('valid_batch_size16'):
        config.DATA.VALID.BATCH_SIZE = args16.valid_batch_size16
    if _check_args('test_batch_size16'):
        config.DATA.TEST.BATCH_SIZE = args16.test_batch_size16
    if _check_args('pretrained16'):
        config.MODEL.PRETRAINED = args16.pretrained16
    if _check_args('resume16'):
        config.MODEL.RESUME = args16.resume16
    if _check_args('output16'):
        config.OUTPUT = args16.output16
    if _check_args('tag16'):
        config.TAG = args16.tag16
...
def get_config16(args16):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args16)

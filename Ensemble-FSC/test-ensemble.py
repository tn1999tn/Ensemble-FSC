...
from config15 import get_config15
from config16 import get_config16
from config17 import get_config17
import os
...
def parse_option():
    ...
    parser15 = argparse.ArgumentParser('Meta-Dataset Pytorch implementation', add_help=False)
    parser15.add_argument('--cfg15', type=str, required=True, metavar="FILE", help='path to config file', )
    parser15.add_argument(
        "--opts15",
        help="Modify config options by adding 'KEY VALUE' pairs. ",
        default=None,
        nargs='+',
    ...
    # easy config modification
    parser15.add_argument('--train_batch_size15', type=int, help="training batch size for single GPU")
    parser15.add_argument('--valid_batch_size15', type=int, help="validation batch size for single GPU")
    parser15.add_argument('--test_batch_size15', type=int, help="test batch size for single GPU")
    parser15.add_argument('--output15', type=str, metavar='PATH',
                          help='root of output folder, the full path is <output>/<model_name>/<tag> (default: output)')
    parser15.add_argument('--is_train15', type=int, choices=[0, 1], help="training or testing")
    parser15.add_argument('--pretrained15', type=str, help="pretrained path")
    parser15.add_argument('--tag15', help='tag of experiment')
    parser15.add_argument('--resume15', help='resume path')

    args15, unparsed15 = parser15.parse_known_args()
    config15 = get_config15(args15)

    parser16 = argparse.ArgumentParser('Meta-Dataset Pytorch implementation', add_help=False)
    parser16.add_argument('--cfg16', type=str, required=True, metavar="FILE", help='path to config file', )
    parser16.add_argument(
        "--opts16",
        help="Modify config options by adding 'KEY VALUE' pairs. ",
        default=None,
        nargs='+',
    ...
    # easy config modification
    parser16.add_argument('--train_batch_size16', type=int, help="training batch size for single GPU")
    parser16.add_argument('--valid_batch_size16', type=int, help="validation batch size for single GPU")
    parser16.add_argument('--test_batch_size16', type=int, help="test batch size for single GPU")
    parser16.add_argument('--output16', type=str, metavar='PATH',
                          help='root of output folder, the full path is <output>/<model_name>/<tag> (default: output)')
    parser16.add_argument('--is_train16', type=int, choices=[0, 1], help="training or testing")
    parser16.add_argument('--pretrained16', type=str, help="pretrained path")
    parser16.add_argument('--tag16', help='tag of experiment')
    parser16.add_argument('--resume16', help='resume path')

    args16, unparsed16 = parser16.parse_known_args()
    config16 = get_config16(args16)

    parser17 = argparse.ArgumentParser('Meta-Dataset Pytorch implementation', add_help=False)
    parser17.add_argument('--cfg17', type=str, required=True, metavar="FILE", help='path to config file', )
    parser17.add_argument(
        "--opts17",
        help="Modify config options by adding 'KEY VALUE' pairs. ",
        default=None,
        nargs='+',
    ...
    # easy config modification
    parser17.add_argument('--train_batch_size17', type=int, help="training batch size for single GPU")
    parser17.add_argument('--valid_batch_size17', type=int, help="validation batch size for single GPU")
    parser17.add_argument('--test_batch_size17', type=int, help="test batch size for single GPU")
    parser17.add_argument('--output17', type=str, metavar='PATH',
                          help='root of output folder, the full path is <output>/<model_name>/<tag> (default: output)')
    parser17.add_argument('--is_train17', type=int, choices=[0, 1], help="training or testing")
    parser17.add_argument('--pretrained17', type=str, help="pretrained path")
    parser17.add_argument('--tag17', help='tag of experiment')
    parser17.add_argument('--resume17', help='resume path')

    args17, unparsed17 = parser17.parse_known_args()
    config17 = get_config17(args17)

    return args, config, config1, config2, config3, config4, config5, config6, config7, config8, config9, config10, config11, config12, config13, config14, config15, config16, config17
...
def test(config, config1, config2, config3, config4, config5, config6, config7, config8, config9, config10, config11,
    ...
    logger15.info(f"Creating model:{config15.MODEL.TYPE}/{config15.MODEL.NAME}")
    logger16.info(f"Creating model:{config16.MODEL.TYPE}/{config16.MODEL.NAME}")
    logger17.info(f"Creating model:{config17.MODEL.TYPE}/{config17.MODEL.NAME}")
    ...
    model15 = get_model(config15).cuda()
    model16 = get_model(config16).cuda()
    model17 = get_model(config17).cuda()
    ...
    if config15.MODEL.PRETRAINED:
        load_pretrained(config15, model15, logger15)

    if config16.MODEL.PRETRAINED:
        load_pretrained(config16, model16, logger16)

    if config17.MODEL.PRETRAINED:
        load_pretrained(config17, model17, logger17)
    ...
    if hasattr(model15, 'mode') and model15.mode == "NCC":
        model15.append_adapter()
    if hasattr(model16, 'mode') and model16.mode == "NCC":
        model16.append_adapter()
    if hasattr(model17, 'mode') and model17.mode == "NCC":
        model17.append_adapter()

    logger.info("Start testing")

    with torch.no_grad():
        acc,ci = testing(config, test_dataset, test_dataloader, model, model1, model2, model3, model4, model5, model6,
                      model7, model8, model9, model10, model11, model12, model13, model14, model15, model16, model17)
...
def testing(config, dataset, data_loader, model, model1, model2, model3, model4, model5, model6, model7, model8, model9,
    ...
    for idx, batches in enumerate(data_loader):
        ...
        ori_labels_no15 = copy.deepcopy(labels)
        ori_imgs_no15 = copy.deepcopy(imgs)
        ori_labels_no16 = copy.deepcopy(labels)
        ori_imgs_no16 = copy.deepcopy(imgs)
        ori_labels_no17 = copy.deepcopy(labels)
        ori_imgs_no17 = copy.deepcopy(imgs)
        ...
        for i, model in enumerate(models):
            ...
            if i != 15:
                max_indices_no15 = torch.argmax(score_no0, dim=2)
                max_indices_no15 = max_indices_no15.view(-1)
                # print(max_indices)
                all_pre_no15.append(max_indices_no15)

            if i != 16:
                max_indices_no16 = torch.argmax(score_no0, dim=2)
                max_indices_no16 = max_indices_no16.view(-1)
                # print(max_indices)
                all_pre_no16.append(max_indices_no16)

            if i != 17:
                max_indices_no17 = torch.argmax(score_no0, dim=2)
                max_indices_no17 = max_indices_no17.view(-1)
                # print(max_indices)
        ...
        for i, pre in enumerate(
                [all_pre_no0, all_pre_no1, all_pre_no2, all_pre_no3, all_pre_no4, all_pre_no5,all_pre_no6, all_pre_no7, all_pre_no8, all_pre_no9,
                 all_pre_no10, all_pre_no11, all_pre_no12, all_pre_no13, all_pre_no14, all_pre_no15, all_pre_no16, all_pre_no17]):
        ...
        score15 = model15.test_forward(ori_imgs_no15, ori_labels_no15, dataset_index)
        score15 = torch.stack(score15)
        score15 = torch.softmax(score15, dim=2)

        score16 = model16.test_forward(ori_imgs_no16, ori_labels_no16, dataset_index)
        score16 = torch.stack(score16)
        score16 = torch.softmax(score16, dim=2)

        score17 = model17.test_forward(ori_imgs_no17, ori_labels_no17, dataset_index)
        score17 = torch.stack(score17)
        score17 = torch.softmax(score17, dim=2)

        score_1=[]
        all_pre_1 = []
        for i, s in enumerate([score, score1,score2, score3, score4, score5, score6, score7,score8, score9,score10, score11, score12, score13, score14, score15, score16,score17]):
            score_1.append(s)
        ...
        pre_no114 = torch.cat((all_pre_1[:14, :], all_pre_1[15:, :]), dim=0)
        pre_no115 = torch.cat((all_pre_1[:15, :], all_pre_1[16:, :]), dim=0)
        pre_no116 = torch.cat((all_pre_1[:16, :], all_pre_1[17:, :]), dim=0)
        pre_no117 = all_pre_1[:17, :]

        q_counts1 = []
        for i, pre in enumerate([pre_no10, pre_no11, pre_no12, pre_no13, pre_no14, pre_no15, pre_no16, pre_no17,
                                 pre_no18, pre_no19, pre_no110, pre_no111, pre_no112, pre_no113, pre_no114, pre_no115,
                                 pre_no116, pre_no117]):
            q_count = torch.tensor(0)
            q_count.cuda()
            s_0 = torch.sum(torch.eq(pre, 0), dim=0)
...
if __name__ == '__main__':
    args, config, config1, config2, config3, config4, config5, config6, config7, config8, config9, config10, config11, config12, config13, config14, config15, config16, config17 = parse_option()
    ...
    torch.cuda.set_device(config15.GPU_ID)
    torch.cuda.set_device(config16.GPU_ID)
    torch.cuda.set_device(config17.GPU_ID)
    ...
    setup_seed15(config15.SEED)
    setup_seed16(config16.SEED)
    setup_seed17(config17.SEED)
    ...
    os.makedirs(config15.OUTPUT, exist_ok=True)
    os.makedirs(config16.OUTPUT, exist_ok=True)
    os.makedirs(config17.OUTPUT, exist_ok=True)
    ...
    logger15 = create_logger(output_dir=config15.OUTPUT, name=f"{config15.MODEL.NAME}")
    logger16 = create_logger(output_dir=config16.OUTPUT, name=f"{config16.MODEL.NAME}")
    logger17 = create_logger(output_dir=config17.OUTPUT, name=f"{config17.MODEL.NAME}")
    ...
    test(config, config1, config2, config3, config4, config5, config6, config7, config8, config9, config10,
             config11, config12, config13, config14, config15, config16, config17)
